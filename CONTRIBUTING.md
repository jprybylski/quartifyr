...
CLI output feeds one of `man/figures/*.gif` -- e.g. `R/render-report.R`
for `render-pipeline.gif` -- without also touching that GIF or its
`dev/tapes/*.tape` recipe. See the hook script itself for the
