...
know or care what happens to its docx afterward.
[`quartifyr::render_report()`](r-package.html) is one convenience
wrapper that chains both passes together; it's optional. See [Using the
pieces directly](#using-the-pieces-directly) below.
...
quartifyr. If you already have your own Quarto project and your own
`reportifyr` project set up, the underlying mechanics are three
ordinary, independent tool calls:
...
- **Word field recalculation** (`styling_recalculate_fields()` /
  ...
  in real testing. Off by default; without it, delivered docs need one
  manual "select all, F9" in Word to populate the ToC. See
  [Styling](styling.html) and [The R package](r-package.html) for
  detail.
