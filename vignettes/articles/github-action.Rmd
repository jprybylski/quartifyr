...
- Its own `renv.lock` (with `reportifyr`/`pyro` pinned, same shape as
  ...
  library (`renv::install("jprybylski/quartifyr")`, from GitHub, not
  from this action's own checkout), it does **not** bring its own
...
`inst/python/` checked out in the calling repo -- installing the
`quartifyr` R package (above) brings all of those along as bundled
package data.
...
3. Installs the `quartifyr` R package itself into the calling project's
   renv library, via `renv::install("jprybylski/quartifyr")` -- pulling
   in `pyro`/`reportifyr` transitively via `quartifyr`'s own
   `DESCRIPTION` `Imports:` if not already present. Note this installs
   from GitHub's default branch, not necessarily the same ref this
   action itself was invoked at (`uses: jprybylski/quartifyr@X`) -- fine
   while there's no tagged release yet, but worth knowing if a caller has
   pinned the action to a stable tag and expects the R package to match.
...
5. Runs `quartifyr::render_report(shell_qmd, status, reference_doc)`
   ...
   action, there's no separate `--toolkit-root` concept anymore: when
   `reference-doc` isn't set, `render_report()`'s own default resolves
   the package's bundled template via `system.file()` -- unless the
   project's own `.qmd`/`_quarto.yml` already configures one, in which
   case that wins instead -- since the package itself is now what's
   installed into the project, not just referenced from an external
   checkout path.
