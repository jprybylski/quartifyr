---
title: "Templated Workflows: Scaffolding Projects with spackle and scaffold"
---
...
new project" walks through this by hand) means copying a handful of
files into a consistent shape: `_extensions/`, a `_quarto.yml` with
`output-dir: report/shell`, a shell `.qmd` with the right frontmatter,
and `reportifyr`'s own `report/standard_footnotes.yaml`/
...
default = "../../inst/templates/org-reference.docx"
...
- **quartifyr already treats "one project = one directory with a fixed
  ...
  `render_report()` depends on; see `vignette("architecture")`). A
  template filler that reproduces that shape exactly, every time,
  removes a whole class of "forgot to set `output-dir`" mistakes.
- **An org's reference-doc rarely changes** once built (see
  `vignette("styling")`), so `reference_doc` is a good candidate for a
...
Then follow the rest of the repo README's "Standing up a new project"
section (installing the extensions, running
`reportifyr::initialize_report_project()` and
`quartifyr::initialize_quartifyr_project()`) against the filled-in
directory.
...
The repo README's "Standing up a new org" section is that shape of task:
copy `inst/python/styles/default.yaml` to `<org>.yaml`, edit it, then
run `styling_build_reference_docx()`/`quartifyr-styling build` with
`override`/`out` pointed at both new paths. It's the same three steps by
hand every time a new org needs a look, unlike "stand up a project,"
which happens once per project.
...
messages:
  pre: |
    Adding a new org style override.
  post: |
    Now build the reference-doc:
      quartifyr-styling build --style /path/to/inst/python/styles/default.yaml \
        --override {{ .Scaffold.org_slug }}.yaml \
        --out {{ .Scaffold.org_slug }}-reference.docx
...
rewrites:
  - from: templates/org-style.yaml
    to: styles/{{ .Scaffold.org_slug }}.yaml
```
